def listarSeleccionProfs(seleccion):
    print("\nSeleccionProfes: ")
    contador = 1
    for cur in seleccion:
        datos="{0}. ID {1} | ID Profesor: {2} | ID Materia: {3} )"
...
def pedirDatosRegistro():
    ...
    idProfesor = int(input("Ingresar id del profesor: "))

    seleccion = (idMateria,idProfesor)
    return seleccion

def pedirDatosEliminacion(seleccion):
    listarSeleccionProfs(seleccion)
    existeCodigo = False
